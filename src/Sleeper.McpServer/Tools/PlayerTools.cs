...
public class PlayerTools
{
    ...
    public static async Task<string> PlayerDeepDive(
        ...
        [Description("Sleeper league ID (needed for league-specific scoring)")] string league_id = "1312539280601522176")
    {
        var dive = await analysis.GetPlayerDeepDiveAsync(league_id, player_name);
        ...
        if (dive.WeeklyPointsBySeason.TryGetValue(recentYear, out var weekly) && weekly.Count > 0)
        {
            ...
            for (int w = 0; w < weekly.Count; w++)
                sb.AppendLine($"- Week {w + 1}: {weekly[w]:F1} pts");
        }
