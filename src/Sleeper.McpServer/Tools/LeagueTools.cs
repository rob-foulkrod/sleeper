...
public class LeagueTools
{
        ...
        [Description("Sleeper league ID")] string league_id = "1312539280601522176")
    {
        ...
        var scoreboard = await sleeperService.GetWeekScoreboardAsync(league_id, week);
        var sb = new StringBuilder();
        ...
        foreach (var m in scoreboard.OrderByDescending(m => (m.Team1Points ?? 0) + (m.Team2Points ?? 0)))
        {
            ...
            var winner = (m.Team1Points ?? 0) >= (m.Team2Points ?? 0) ? t1 : t2;
            sb.AppendLine($"- **{t1}** {m.Team1Points:F2} vs {m.Team2Points:F2} **{t2}** (Winner: {winner})");
        }
    ...
    public static async Task<string> ScorePlayerWeek(
        ...
        [Description("Sleeper league ID")] string league_id = "1312539280601522176")
    {
        var score = await fantasyService.GetPlayerWeekScoreAsync(league_id, player_id, season, week);
        ...
        [Description("Maximum rounds to show (0 for all)")] int max_rounds = 5)
    {
        ...
        if (max_rounds > 0 && maxRd < (picks.Max(p => p.Round)))
            sb.AppendLine($"\n*Showing first {max_rounds} rounds of {picks.Max(p => p.Round)}*");
