...
public class AnalysisService : IAnalysisService
{
    ...
    public async Task<KeeperReport> GetKeeperAnalysisAsync(string leagueId, string username, int historyYears = 3, CancellationToken ct = default)
    {
        ...
        var replacementLevels = rankings.CalculateReplacementLevels(config.Teams, config.StarterSlots, config.FlexSlots);
    ...
    private static int GetLastCompletedSeason(NflState? nflState, int currentSeason)
    {
        if (nflState is null) return currentSeason - 1;
        var lastCompleted = int.TryParse(nflState.PreviousSeason, out var prev) ? prev : currentSeason - 1;
        if (nflState.SeasonType == "off") lastCompleted = prev;
